# McLeod API Documentation - /dispatch/{stopId}/endTrip

**Source URL:** https://tms-cfaa.loadtracking.com:5690/ws/docs/services?operation=handleEndTrip&role=-1&service=DispatchService
...
Handles the changes needed to be made when a driver ends a trip. Currently, this only works for LTL peddle movements since these are the only ones that use begin and end trip stops.
...
---|---|---|---|---  
stopId | the ID of the end trip stop to mark arrived (not currently used) |  path  |  |  [String](https://tms-cfaa.loadtracking.com:5690/ws/docs/types?type=java.lang.String&role=-1)  
...
**Endpoint:** `POST /dispatch/{stopId}/endTrip`
...
```http
POST /dispatch/{stopId}/endTrip HTTP/1.1
